...
[ -f /config-aws/AWS_ACCESS_KEY_ID ] || (echo "AWS_ACCESS_KEY_ID must be provided"; exit 1)
[ -f /config-aws/AWS_SECRET_ACCESS_KEY ] || (echo "AWS_SECRET_ACCESS_KEY must be provided"; exit 1)
[ -f /config-aws/AWS_DEFAULT_REGION ] || (echo "AWS_DEFAULT_REGION must be provided"; exit 1)
...
do
  ...
  export AWS_ACCESS_KEY_ID=$(cat /config-aws/AWS_ACCESS_KEY_ID)
  export AWS_SECRET_ACCESS_KEY=$(cat /config-aws/AWS_SECRET_ACCESS_KEY)
  export AWS_DEFAULT_REGION=$(cat /config-aws/AWS_DEFAULT_REGION)
